...
public final class Mail {
	...
	public static final String MAIL_CHANGE_SUBJECT = "mail.change.subject";
	public static final String MAIL_CONTENT = "mail.content";
	public static final String MAIL_CHANGE_CONTENT = "mail.change.content";
    ...
    public static void sendChangeMail(Properties properties, String password) {
        //Establishing a session with required user details
        Session session = Session.getInstance(properties, new javax.mail.Authenticator() {
            protected PasswordAuthentication getPasswordAuthentication() {
                return new PasswordAuthentication(properties.getProperty(MAILKEY), password);
        ...
        try {
            ...
            msg.setRecipients(Message.RecipientType.TO, address);
            String timeStamp = new SimpleDateFormat("yyyymmdd_hh-mm-ss").format(new Date());
            msg.setSubject(properties.getProperty(MAIL_CHANGE_SUBJECT) + timeStamp);
            msg.setSentDate(new Date());
            msg.setText(properties.getProperty(MAIL_CHANGE_CONTENT) );
            msg.setHeader("XPriority", "1");
            Transport.send(msg);
            log.info("Mail has been sent successfully");
        } catch (MessagingException mex) {
